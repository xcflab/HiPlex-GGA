...
#pETCON adaptors:
yeast_primer5 = five_prime_addition_5[adapter_idx]# petcon: "GGGTCGGCTTCGCATATG"
yeast_primer3 = three_prime_addition_3[adapter_idx] # petcon: "CTCGAGGGTGGAGGTTCC"
...
for design in designs:
    ...
    for trial in range(1000):
        ...
        gc = gc_fraction(overlapsegment) * 100
    ...
    #assert str(Seq(seq, unambiguous_dna).translate()) == protein_seqs[design]
    skip = False
    tm = MeltingTemp.Tm_NN(overlapsegment, Na=50, K=0, Tris=0, Mg=args.MgmM, dNTPs=args.dNTPsmM ); gc = gc_fraction(overlapsegment) * 100; overlapsegment_len = len(overlapsegment)
    ...
    if (len(missaligned1) > 1) and not skip:
        ...
        if fixed:
            ...
            #assert str(Seq(seq, unambiguous_dna).translate()) == protein_seqs[design]
            Tm = MeltingTemp.Tm_NN(overlapsegment, Na=50, K=0, Tris=0, Mg=args.MgmM, dNTPs=args.dNTPsmM )
            GC_cont = gc_fraction(overlapsegment) * 100
